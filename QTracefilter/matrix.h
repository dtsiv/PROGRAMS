...
private:
    ...
    {   T **Val;
	    ...
	    {   Row = row; RowSiz = row;
	        ...
	        for(size_t i=0; i < row; i++) Val[i] = new T [col];
	    }
	    ~base_mat ()
...
public:
    static base_mat * MatrixCach[MAXMATRIXSISE][MAXMATRIXSISE];
	static CRITICAL_SECTION LockCach;
	static void InitMatrixCach(void);
    static void DeleteMatrixCach(void);
};

MAT_TEMPLATE inline
void matrixT::InitMatrixCach()
{   int i, j;

    InitializeCriticalSection(&LockCach);
    i = 0;
    EnterCriticalSection(&LockCach);
    while(i < MAXMATRIXSISE)
    {   j = 0;
        while(j < MAXMATRIXSISE) MatrixCach[i][j++] = NULL;
        i++;
    }
    LeaveCriticalSection(&LockCach);
}

MAT_TEMPLATE inline
void matrixT::DeleteMatrixCach()
{   int i, j;
    ...
    while(i < MAXMATRIXSISE)
    {   j = 0;
        while(j < MAXMATRIXSISE)
        {   base_mat * pm = MatrixCach[i][j++];
            int num = 0;

            while(pm != NULL)
            {   base_mat * pm_ = pm -> pNext;
                delete pm;
                pm = pm_;
                num++;
            }
            if(num)
				DbgPrint(L"math::matrix: DeleteCach row=%ld col=%ld num=%ld", i, j - 1, num);
        }
        i++;
    }
    LeaveCriticalSection(&LockCach);
	DeleteCriticalSection(&LockCach);
}
...
{   base_mat ** ppm = &MatrixCach[row][col];

    EnterCriticalSection(&LockCach);
    if(*ppm == NULL)
    {	LeaveCriticalSection(&LockCach);
        _m = new base_mat(row, col);
    } else
    {   _m = *ppm;
        *ppm = _m -> pNext;
        LeaveCriticalSection(&LockCach);
...
matrixT::clone ()
{   base_mat * pm;
    base_mat ** ppm = &MatrixCach[_m -> Row][_m -> Col];

    EnterCriticalSection(&LockCach);
    if(*ppm == NULL)
    {   LeaveCriticalSection(&LockCach);
        pm = new base_mat(_m -> Row, _m -> Col);
    } else
    {   pm = *ppm;
        *ppm = pm -> pNext;
        LeaveCriticalSection(&LockCach);
    ...
    for(size_t i = 0; i < _m -> Row; i++)
        memcpy(pm -> Val[i], _m -> Val[i], _m -> Col * sizeof(T));

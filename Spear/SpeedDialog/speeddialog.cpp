...
SpeedDialog::SpeedDialog(QWidget *parent, Qt::WindowFlags flags)
	...
	, m_iReceiveDelay(0)
{
	...
	ui.startButton->setEnabled(true);
        ...
        ui.plainTextEdit->appendPlainText(QT_VERSION_STR);
...
#pragma GCC diagnostic pop
	...
	// receiver 
    m_pReceiver = new Receiver(0,m_iReceiveDelay);
...
void SpeedDialog::clean() {
	...
	if (m_pConnection) { 
		...
		qDebug() << "m_bServer=" << m_bServer
                 << " waitForDisconnected (30 sec)="
				 << m_pConnection->waitForDisconnected(SD_WAIT_TIMEOUT);
	}
	...
	if (m_senders.count()) {
		...
		while (!m_threads.isEmpty()) {
			...
			if (pThread->isRunning())  {
                if (!pThread -> wait(SD_WAIT_TIMEOUT)) qDebug() << "pThread->wait(30 sec)=false";
			}
			delete pThread;
		}
	}
	...
	if (m_pReceiveThread) {
		if (m_pReceiveThread->isRunning())  {
            if (!m_pReceiveThread -> wait(SD_WAIT_TIMEOUT)) qDebug() << "m_pReceiveThread->wait(30 sec)=false";
		}

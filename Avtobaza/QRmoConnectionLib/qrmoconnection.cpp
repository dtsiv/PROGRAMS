...
QRmoConnection::~QRmoConnection() {
    ...
    // delete the QRmoConnectionPrivate object dynamically allocated fields and the object itself
...
void QRmoConnection::doDisconnect() {
	...
	if (d->m_pConnectionThreadWorker->isConnected()) {
		// just emits cleanupBlocking() (connection type Qt::AutoConnection)
		// and returns. User gets informed by disconnected() signal
